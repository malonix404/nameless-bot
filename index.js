...
const { initializeTempSystem } = require('./lib/tempManager');
...
async function startBot() {
  ...
  // Check if sessionID is provided and process NamelessBot! format session
  if (settings.sessionID && settings.sessionID.startsWith('NamelessBot!')) {
    try {
      const [header, b64data] = settings.sessionID.split('!');

      if (header !== 'NamelessBot' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'NamelessBot!.....'");
      }
      ...
      console.log('💜 Session : NamelessBot Session');

    } catch (e) {
      console.error('💜 Session : ❌ Error processing NamelessBot session:', e.message);
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log(`👑 Owner: Prince Malone\n`);
